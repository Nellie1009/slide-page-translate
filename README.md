...
配图与简洁修订的新规范见 [逐页配图与简洁修订](references/visual-notes.md)：逐页覆盖、按需找现成中文图（可含个人笔记）、不匹配就不用、去重而不漏译、纠错与译文分开。默认仍是完整翻译；用户明确选择讲解版时须另行标明。当前脚本尚不支持外部图片入栏和讲解模式，规范不等于已新增这些功能；具体限制及多 PDF 合并核对见该参考。
