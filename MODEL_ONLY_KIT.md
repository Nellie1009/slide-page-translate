...
默认完整逐页翻译；“太复杂”先去重复图卡、说明和边框，不能自行改成摘要。只有用户明确选择讲解版才改变范围，并如实标注。逐页建立“源文件＋实际页号”的知识点清单，不用少数总览模块代替全稿。

用户优先找图时，按本页对象、机制、坐标与关系寻找并核对现成中文教材图或个人笔记；不因主题相近混用，不以官方身份代替正确性与可读性判断。用户禁止自绘时不填 diagram；找不到匹配图就说明缺口，不凑图。图直接放中文侧，保留必要图例和来源，不再嵌一张双栏旧页面，不重复贴原图或循环复用总览卡。使用图片前核对许可；未获相应使用依据时改用来源链接及文字说明。

原文疑似错误另列“原文如此／待核实”或经核实的译者注，补充说明与原文分开；放大原页后才判断无法辨认。最终检查所有页的对应和排版，并放大每张新图、曲线、公式、表格及纠错页，不能只靠程序通过。

本工具包代码尚不支持外部图片入栏、讲解模式或多文件合并命令；不得编造 image_path 等字段或改校验使摘要过关。必需这些功能时向用户说明执行缺口；具备开发授权后扩展原脚本并测试，未实现前不宣称完成。多份课件分别校验导出，再用可用 PDF 工具合并并核对每个源页及续页映射。纯聊天端不能假称已下载图片或已生成 PDF。
